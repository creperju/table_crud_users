...
class HTML_Tag extends \core\Clase_Base {
	...
	(
		...
		$uri = \core\URL::http_generar($query_string);
                ...
                /*$onclick = "onclick="."window.location.assign('".\core\URL::generar($controlador)."')";
                $onmouseover = "onmouseover="."cambiar_btn_menu('$controlador','in');";
                $onmouseout = "onmouseout="."cambiar_btn_menu('$controlador','out');";
                
                $link = '<div class="'.$clases.'" id="btn_'.$controlador.'"
                            '.$onclick.'
                            '.$onmouseover.'
                            '.$onmouseout.'
                            >
			<img class="img_btn_menu" id="img_'.$controlador.'" src="'.URL_ROOT.'recursos/imagenes/btn_'.$controlador.'_inactivo.gif"/>
			<span>'.$texto.'</span>
		    </div>';
	...
	/**
	 * Genera un elemento &lt;li&gt; para ser usado en un ménu si el usuario tiene permisos para ejecutar el controlador y métodos indicados en la query string para la aplicacion table-crud-users
	 * 
	 * @param string $clases Ejemplo: "clase1 clase2 clase3 ..."
	 * @param array $query_string Ejemplo: array("controlador", "metodo") array("articulos", "index")
	 * @param string $texto Ejemplo: "Artículos"
	 ...
	 * @return string
	 */
	public static function div_menu
	(
			$clases,
			array $query_string = array("inicio", "index"),
			$texto,
	...
	public static function a_boton(
		...
		$uri = \core\URL::http_generar($query_string);
                ...
                $onmouseover = "onmouseover = "."this.style.color='yellow';"."this.style.backgroundColor='black';";
                $onmouseout = "onmouseout = "."this.style.color='white';"."this.style.backgroundColor='#555555';";
                
		$link = "<a class='$clases' href='$uri' $argumentos $onmouseover $onmouseout>$texto</a>";
		return $link;
