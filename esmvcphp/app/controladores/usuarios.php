...
class usuarios extends \core\Controlador {
	...
	public function form_login_validar(array $datos = array()) {
		...
		if (\core\Usuario::$login != "anonimo") {
			...
			$datos["mensaje"] = "<h3>Ya te encuentras conectado</h3> Utiliza el menú para navegar.";
		...
		elseif ( ! \core\HTML_Tag::form_autenticar("form_login", "post")) {
			...
			$datos["mensaje"] = "<h3>Error: formulario no identificado.</h3>";
...
//							\core\Distribuidor::cargar_controlador("usuarios", "form_login", $datos);
				...
				if ($validacion) {
					...
					if( $respuesta == 'no_existe' ){
                                                ...
                                                $datos['mensaje'] = 'El usuario no existe. <a href="'. \core\URL::generar("usuarios/form_login").'">Volver</a>';
                                                \core\Distribuidor::cargar_controlador("mensajes", "mensaje", $datos);
                                        }
					elseif  ($respuesta == 'existe') {
						//$datos['errores']['validacion'] = 'Error en login o password';
                                                
                                                $datos['mensaje'] = 'La contraseña no es válida. <a href="'. \core\URL::generar("usuarios/form_login").'">Volver</a>';
					...
					elseif ($respuesta == 'existe_autenticado_confirmado') {
						...
						//$this->cargar_controlador('mensajes', 'mensaje', $datos);
                                                //$datos['inicio_usuario'] = $datos['values']['login'];
                                                
                                                // Cambia la URL estéticamente para que se quede en app/
                                                \core\HTTP_Respuesta::set_header_line("Location", \core\URL::generar());
                                                \core\Distribuidor::cargar_controlador("inicio", "index", $datos);
	...
	public function desconectar(array $datos = array()) {
		...
		if ($datos['desconexion_razon'] === null) {
			$datos['mensaje'] = 'Fin de la sesión, adiós.';
		}
